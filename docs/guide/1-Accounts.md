...
First, let's start by creating the accounts (if you already have them, you can skip these steps):

1. Create a free [**Trakt**](http://www.trakt.tv) account.
   * *This is recommended for tracking what you watch, and getting some custom lists. Makes the Stremio/Nuvio experience more like Netflix. If your account is new, rate or mark as watched at least 10 movies and 10 shows, it will be good for creating custom lists later below, otherwise they won't work.*
2. Create a [**TMDB**](https://www.themoviedb.org/) account and get a free API key:
   * *TMDB is used for the metadata (descriptions, cast, etc.) of Movies.*
   ...
   ![TMDB API Key](../images/1.2.png)

3. Create a [**TVDB**](https://www.thetvdb.com/) account and get a free API key:
   * *TVDB is used for the metadata (descriptions, cast, etc.) of Series.*
   ...
   ![TVDB API Key](../images/1.3.png)

4. Create a **Google/Gmail** account if you don't have one, and get a free **Gemini** API key:
   ...
   * *If no key is created automatically, just create one by clicking "Create API Key" on the top right.*
   ...
   ![Gemini API Key](../images/1.4.png)

5. Choose a Debrid service for caching the torrents, create an account on it, buy a subscription, and get the API key.
   ...
   * *I use mainly* [***TorBox***](https://torbox.app/subscription?referral=19c21001-d6fe-4b66-952c-8adf4832dd66), as a backup I use [***Real-Debrid***](http://real-debrid.com/?id=12639093) *(these are referral links since we're at it :), two of the best platforms, with the best prices and very stable.* **DON'T USE REAL-DEBRID FOR NOW, IT'S HAVING LEGAL ISSUES, PROCEED WITH TORBOX.**
   ...
   * *ONLY AFTER you registered to one or both services from the links above and subscribed to a plan (and paid for it), you can get the **API key** while logged in to your account directly on [**this**](https://real-debrid.com/apitoken) link for Real-Debrid or [**here**](https://torbox.app/settings?section=account) for TorBox.*
   ...
   ![TorBox & Real-Debrid API Key](../images/1.5.2.png)

6. (**Optional**) Totally optional and not a must at all, but if Trakt feels limiting with its rate limits or you want more curated lists, check out these friendly alternatives that work great with AIOMetadata:
